...
class HTTPServer(object):
	...
	def handle_request(self, connfd):
		...
		#获取请求行
		request_list = request_lines[0].decode()
		
		#正则提取请求方法和请求内容
		pattern = r"(?P<METHOD>[A-Z]+)\s+(?P<PATH>/\S*)"
		try:
			env = re.match(pattern, request_list).groupdict()
		except:
	...
	def send_request(self, method, path):
		...
		response_body = s.recv(4096*16).decode()
	...
	def get_headlers(self, status):
		if status == "200":
			...
			response_headlers = "HTTP/1.1 400 Not Fount\r\n"
			response_headlers += "\r\n"
